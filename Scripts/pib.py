...
import numpy as np
# allo vs indica
for n in range(1,13):#alloc9_intergenicDiversity.mafs indicach6_intergenicDiversity.filtered.mafs
	input1="~/rice/alloch"+str(n)+"_intergenicDiversity.filtered.mafs"
	input3="~/rice/sympch"+str(n)+"_intergenicDiversity.filtered.mafs"
	out1="~/rice/as_"+str(n)+".txt"
	
	a = pd.read_table(input1, usecols=['chromo','position','major','knownEM','nInd'])
	a=a[a.nInd == 4]
	a['a.maf']=a['knownEM']
	a['a.major']=a['major']
	a=a.drop('knownEM',1)
...
# symp vs indica
#for n in range(1,13):
#        input2="~/rice/sympch"+str(n)+"_intergenicDiversity.filtered.mafs"
#        input3="~/rice/indicach"+str(n)+"_intergenicDiversity.filtered.symp.mafs"
#        out2="~/rice/si_"+str(n)+".txt"
